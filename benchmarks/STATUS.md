...
| [Field scan v1](field-scan-v1/README.md) | Complete; 11 of 12 pinned repositories finished with 0 P0 — but the twelfth reports 294 confirmed `#3` hits, so the zero describes what completes, not public code | That the deterministic tier, run under a rule frozen before the scan, decides nothing on its own on popular public code; and that one very large repository still exceeds the per-repository budget | Any detection claim for the deterministic tier, and any recall estimate |
