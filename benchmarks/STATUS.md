...
| [Field scan v1](field-scan-v1/README.md) | Complete; a null result — 10 of 12 pinned repositories finished, 0 P0 hits | That the deterministic tier, run under a rule frozen before the scan, decides nothing on its own on popular public code; and that two very large repositories still exceed the per-repository budget | Any detection claim for the deterministic tier, and any recall estimate |
